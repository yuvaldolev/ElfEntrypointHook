...
ElfImg::ElfImg(std::string_view base_name) : elf(base_name) {
  ...
  char *section_str =
      offsetOf<char *>(header, section_header[header->e_shstrndx].sh_offset);
    ...
    switch (section_h->sh_type) {
      case SHT_DYNSYM: {
        if (bias == -4396) {
          ...
          dynsym_start =
              offsetOf<decltype(dynsym_start)>(header, dynsym_offset);
        }
        break;
      }
      case SHT_SYMTAB: {
        if (strcmp(sname, ".symtab") == 0) {
          ...
          symtab_start =
              offsetOf<decltype(symtab_start)>(header, symtab_offset);
        }
        break;
      }
      case SHT_STRTAB: {
        if (bias == -4396) {
          ...
          strtab_start =
              offsetOf<decltype(strtab_start)>(header, symstr_offset);
        }
      ...
      case SHT_GNU_HASH: {
        ...
        gnu_bucket_ = reinterpret_cast<decltype(gnu_bucket_)>(
            gnu_bloom_filter_ + gnu_bloom_size_);
    ...
    if (symtab_start != nullptr && symstr_offset_for_symtab != 0) {
      for (ElfW(Off) i = 0; i < symtab_count; i++) {
        ...
        const char *st_name = offsetOf<const char *>(
            header, symstr_offset_for_symtab + symtab_start[i].st_name);
...
bool ElfImg::findModuleBase() {
  ...
  while ((nread = getline(&buff, &len, maps)) != -1) {
    ...
    if ((contains(line, "r-xp") || contains(line, "r--p")) &&
        contains(line, elf)) {
